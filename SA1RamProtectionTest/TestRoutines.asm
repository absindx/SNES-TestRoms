...
+
		LDA	!Sa1MessageByte
		STA.w	!TestSa1StackPointerBootL, Y
		STA	!TestActuals, X
		JMP	TestSnesExecute
...
+
		LDA	!Sa1MessageByte
		STA.w	!TestSa1StackPointerBootH, Y
		STA	!TestActuals, X
		JMP	TestSnesExecute
...
GetStackPointeTestID:
		...
		LDY.b	#$00
		RTS
+		CMP.b	#!TestID_SA1_Reboot_SPH+1	;   !TestID_SA1_Reboot_SPL
		BCS	+
		LDA.b	#!TestID_SA1_Reboot_SPL
		LDY.b	#$04
		RTS
+		;LDA.b	#$FF				;   Invalid
		RTS
...
!BwRamProtectOrder_C	= 1
!BwRamProtectOrder_S	= 2
!BwRamProtectOrder_B	= 3
...
;   X   = BWPA
;   Y   = Order
;   P.V = target (0: SNES, 1: SA-1)
TestBwRamProtectOrder_Sa1:
		; .shortm, .shortx
		%SendSA1MessageAcc(!Message_TestReady)
...
.OrderS		DEC	A
		BNE	.OrderB
		LDA.b	#!BwRamProtectOrder_S
		%SendSA1MessageAcc(!Message_TestSnesExecute)
		BRA	.LoopNext
.OrderB		TXA
		ORA.b	#!BwRamProtectOrder_B
		%SendSA1MessageAcc(!Message_TestSnesExecute)
		;BRA	.LoopNext
.LoopNext	TYA
		LSR
...
.Check		%SendSA1Message(!Message_TestCheck, $00)
...
TestBwRamProtectOrder_Snes:
		SEP	#$30
		; .shortm, .shortx, CLV

		LDA	!Sa1MessageByte			;\
		LSR					; | A = !Sa1MessageByte low  nibble
		LSR					; | X = !Sa1MessageByte high nibble
		LSR					; |   BWPA
		LSR					; |
		TAX					; |
		LDA	!Sa1MessageByte			; |
...
+		JSR	WriteTestBwRamSnesExpected
		...
		;%TestReturn_SNES()
		TYA
		CMP.b	#!TestID_SNES_BwRamProtectReset_CeSe
		BEQ	.CancelNext
		INC	A
		STA	!TestingID
		JMP	TestSnesExecute
...
;--- BW-RAM Mirroring
macro	TestSnesBwRamMirror(id, addr, sa1Expected, snesExpected)
%TestPattern_ID(<id>)	;-----
%TestPattern_SA1()
...
macro	TestSa1BwRamMirror(id, addr, sa1Expected, snesExpected)
%TestPattern_ID(<id>)	;-----
%TestPattern_SA1()
		%SoftwareJSR(ClearBwRam)
...
%TestPattern_ID(!TestID_SA1_BwRamProtectMirror_806000)	;-----
%TestPattern_SA1()
		SEP	#$70
		; .shortm, .shortx, SEV
...
?TestBwRamProtectMappingSa1:
	...
	if <target> == 0
		...
		%SendSA1MessageAcc(!Message_TestSa1BwRamExpectedE)
...
?TestBwRamProtectMappingSnes:
	...
	if <target> == 0
		...
		; SEV
?.Disable	LDA.b	#$00
?.Send		JSR	WriteTestBwRamSnesActual
...
%TestBwRamProtectMapping(!TestID_SNES_BwRamProtectMapping_Enable, 0, 1, $00, 1, 1, $006000)
%TestBwRamProtectMapping(!TestID_SNES_BwRamProtectMapping_M00P,   0, 0, $00, 0, 0, $0060FF)
%TestBwRamProtectMapping(!TestID_SNES_BwRamProtectMapping_M00E,   0, 0, $00, 1, 1, $006100)
%TestBwRamProtectMapping(!TestID_SNES_BwRamProtectMapping_M01,    0, 0, $01, 1, 1, $006100)
%TestBwRamProtectMapping(!TestID_SA1_BwRamProtectMapping_Enable,  1, 1, $00, 1, 1, $006000)
%TestBwRamProtectMapping(!TestID_SA1_BwRamProtectMapping_M00P,    1, 0, $00, 0, 0, $0060FF)
%TestBwRamProtectMapping(!TestID_SA1_BwRamProtectMapping_M00E,    1, 0, $00, 1, 1, $006100)
%TestBwRamProtectMapping(!TestID_SA1_BwRamProtectMapping_M01,     1, 0, $01, 1, 1, $006100)
...
macro	TestBwRamProtectOrder(id, target, o1, o2, o3, setBWPA)
%TestPattern_ID(<id>)	;-----
%TestPattern_SA1()
	if <target> == 0
		; Target: SNES
		...
		LDA.b	#<id>
		LDX.b	#<setBWPA>
		LDY.b	#((!BwRamProtectOrder_<o1>)+(!BwRamProtectOrder_<o2><<2)+(!BwRamProtectOrder_<o3><<4))

		JMP	TestBwRamProtectOrder_Sa1


%TestPattern_SNES()
		JMP	TestBwRamProtectOrder_Snes
endmacro

%TestBwRamProtectOrder(!TestID_SNES_BwRamProtectOrder_CSB, 0, C, S, B, $01)
%TestBwRamProtectOrder(!TestID_SNES_BwRamProtectOrder_CBS, 0, C, B, S, $02)
%TestBwRamProtectOrder(!TestID_SNES_BwRamProtectOrder_SCB, 0, S, C, B, $03)
%TestBwRamProtectOrder(!TestID_SNES_BwRamProtectOrder_SBC, 0, S, B, C, $04)
%TestBwRamProtectOrder(!TestID_SNES_BwRamProtectOrder_BCS, 0, B, C, S, $05)
%TestBwRamProtectOrder(!TestID_SNES_BwRamProtectOrder_BSC, 0, B, S, C, $06)
%TestBwRamProtectOrder(!TestID_SA1_BwRamProtectOrder_CSB,  1, C, S, B, $01)
%TestBwRamProtectOrder(!TestID_SA1_BwRamProtectOrder_CBS,  1, C, B, S, $02)
%TestBwRamProtectOrder(!TestID_SA1_BwRamProtectOrder_SCB,  1, S, C, B, $03)
%TestBwRamProtectOrder(!TestID_SA1_BwRamProtectOrder_SBC,  1, S, B, C, $04)
%TestBwRamProtectOrder(!TestID_SA1_BwRamProtectOrder_BCS,  1, B, C, S, $05)
%TestBwRamProtectOrder(!TestID_SA1_BwRamProtectOrder_BSC,  1, B, S, C, $06)

;--- Wrong register
%TestPattern_ID(!TestID_SNES_CIWP)	;-----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
%TestPattern_ID(!TestID_SNES_CBWE)	;-----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
		...
		%SendSA1Message(!Message_TestReady, !TestID)
		%SendSA1Message(!Message_TestSa1BwRamExpectedE, %00000000)
		%SendSA1Message(!Message_TestSa1BwRamExpectedA, %00000000)
		%SendSA1Message(!Message_TestSnesExecute, %10000000)
...
%TestPattern_SNES()
		...
		JSR	WriteTestBwRamSnesExpected
		JSR	TestBwRam
		JMP	WriteTestBwRamSnesActual

%TestPattern_ID(!TestID_SA1_SIWP)	;-----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
.Call1		LDA.b	#%00000000
		STA	!SA1_SIWP
		JMP	WriteTestIRamSnesExpected

.Call2		JSR	TestIRam
		JSR	WriteTestIRamSnesActual
		JMP	TestResultCheck

%TestPattern_ID(!TestID_SA1_SBWE)	;-----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
.Call1		CLV
		...
		;LDA.b	#%00000000
		STA	!SA1_BWPA
		JMP	WriteTestBwRamSnesExpected

.Call2		JSR	TestBwRam
		JSR	WriteTestBwRamSnesActual
		JMP	TestResultCheck

%TestPattern_ID(!TestID_SA1_BWPA)	;-----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
%TestPattern_SNES()
	...
	if <id> != !TestID_SNES_BwRamProtectReset_CeSe
		JMP	TestSnesBwRamProtection_SnesStop
	else
		JSR	TestSnesBwRamProtection_SnesStop

		LDA.b	#%00000000			;\
		STA	!SA1_CBWE			; | for !TestID_SNES_BwRamProtection_Reboot
		LDA.b	#%00000010			; |
		STA	!SA1_BWPA			;/
...
;--- SA-1 Reboot
%TestPattern_ID(!TestID_SA1_Reboot_SPL)	;-----
%TestPattern_SA1()
		; NOP
...
%TestPattern_SNES()
		...
		LDX	!Sa1MessageType
		CPX.b	#!Message_Boot_SPL
		BEQ	.Received

.Reboot		LDA.w	#SA1RESET_Reboot		;\
		STA	!SA1_CRVL			; | reboot SA-1
		LDX.b	#%00000000			; |
		STX	!SA1_CCNT			;/
		RTS
...
%TestPattern_ID(!TestID_SA1_Reboot_SPH)	;-----
TestPattern_SA1_Reboot:
%TestPattern_SA1()
		%TestReturn_SA1()
...
%TestPattern_ID(!TestID_SNES_IRamProtection_Reboot)	;-----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
%TestPattern_ID(!TestID_SNES_BwRamProtection_Reboot)	;----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
%TestPattern_ID(!TestID_SA1_IRamProtection_Reboot)	;----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx
...
%TestPattern_ID(!TestID_SA1_BwRamProtection_Reboot)	;----
%TestPattern_SA1()
		SEP	#$30
		; .shortm, .shortx

		%SendSA1Message(!Message_TestReady, !TestID)
		%SendSA1Message(!Message_TestSa1BwRamExpectedE, $80)
		%SendSA1Message(!Message_TestSa1BwRamExpectedA, $00)
		%SoftwareJSR(TestBwRam)
		%SendSA1MessageAcc(!Message_TestSa1BwRamActualA)
		LDA.b	#$00
		BVC	+
