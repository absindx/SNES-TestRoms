...
CartridgeInformation:
	...
	db	"SA-1 REBOOT TEST"			; $00FFC0 : Game title
	pad $00FFD5
...
;							; | B-Bus address = !PPU_VMDATAL
;		STA	!DMA_DMAP0			;/    with !DMA_BBAD0
;		LDA.w	#(.Tilemap+!TilemapOffset)	;\
;		STA	!DMA_A1T0L			; | A-Bus address = .Tilemap
;		LDX.b	#((.Tilemap+!TilemapOffset)>>16); |
;		STX	!DMA_A1B0			;/
...
InitializeTilemap_Main:
		...
		LDX.w	#.Tilemap
		LDY.w	#!TilemapBuffer
		MVN	bank(.Tilemap), bank(!TilemapBuffer)	; src, dst
...
SA1RESET:
		; save registers
		STA.l	!SA1_SIVL
		STA.l	!TestRegisterTmp_AL
		STZ	!TestSa1MemoryLengthTest	;   expects DB register to be $00
		SEP	#$20
		; .shortm
		BEQ	.NxZ1
.NxZ0		BMI	.N1Z0
.N0Z0		LDA.b	#(%0000000000)|(%0000000000)
		BRA	.CheckC
.N1Z0		LDA.b	#(!Sa1SFlag_N)|(%0000000000)
		BRA	.CheckC
.NxZ1		BMI	.N1Z1
.N0Z1		LDA.b	#(%0000000000)|(!Sa1SFlag_Z)
		BRA	.CheckC
.N1Z1		LDA.b	#(!Sa1SFlag_N)|(!Sa1SFlag_Z)
		;BRA	.CheckC
.CheckC		BCC	.CheckV
		ORA.b	#!Sa1SFlag_C
.CheckV		BVC	.CheckEnd
		ORA.b	#!Sa1SFlag_V
.CheckEnd	ORA.b	#!Sa1CiwpMask
		STA.l	!SA1_CIWP			;   disable SA-1 I-RAM write protection
		AND.b	#%11110000
		STA.l	!TestSa1StatusTemporary
		XBA
		STA.l	!TestRegisterTmp_AH
		LDA.b	#$00
		CLC
		XCE
		ROL
		STA.l	!TestRegisterTmp_PH
		REP	#$20
		; .longm
		TSC
		STA.l	!TestRegisterTmp_S
		LDA.w	#$01FF
		TCS
		PHP
...
.SetPBR		PEA	$0000
		...
		XBA
		AND.b	#%00011100
		STA	!TestRegisterTmp_PL		;   ???xdi??
		LDA	!TestSa1MemoryLengthTestH
		BEQ	.MClear
		LDA.b	#%00100000
.MClear		TSB	!TestRegisterTmp_PL		;   ??mxdi??
		LDA	!TestSa1StatusTemporary		;   nvzc0000
		LSR
		LSR
		LSR
		LSR
		AND.b	#%00000011			;   000000zc
		ORA	!TestSa1StatusTemporary		;   nvzc00zc
		AND.b	#%11000011			;   nv0000zc
		TSB	!TestRegisterTmp_PL		;   nvmxdizc
		TDC
		STA	!TestRegisterTmp_D
		REP	#$30
		LDA.w	#$0000
		TCD
		; .longm, .longx
		TXA
		STA	!TestRegisterTmp_X
		TYA
		STA	!TestRegisterTmp_Y
		TDC					;   A = $0000

		SEP	#$20
		; .shortm, .longx
		LDX	!SA1_MR1
		STX	!TestRegisterTmp_MR1
		LDX	!SA1_MR3
		STX	!TestRegisterTmp_MR3
		LDX	!SA1_MR5			;   MR5 + OF
		STX	!TestRegisterTmp_MR5
		LDX	!SA1_VDPL
		STX	!TestRegisterTmp_VDP
